...
data1 <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/COMUNA_C17.shp"))})
lapply(data1, function(x) {simplygeojson(x, "Comunas", 0.2)})

data2 <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/LIMITE_URBANO_CENSAL_C17.shp"))})
lapply(data2, function(x) {simplygeojson(x, "Limite_Urbano", 0.2)})

data3 <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/PROVINCIA_C17.shp"))})
lapply(data3, function(x) {simplygeojson(x, "Provincias", 0.2)})

data4 <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/MANZANA_IND_C17.shp"))})
lapply(data4, function(x) {simplygeojson(x, "Manzanas", 0.2)})

data5 <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/REGION_C17.shp"))})
lapply(data5, function(x) {simplygeojson(x, "Regiones", 0.2)})

data6 <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/DISTRITO_C17.shp"))})
lapply(data6, function(x) {simplygeojson(x, "Distritos", 0.2)})

data7 <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/ZONA_C17.shp"))})
lapply(data7, function(x) {simplygeojson(x, "Zonas", 0.2)})


chl <- lapply(paste0("R",sprintf("%02d",1:15)), function(x){read_sf(paste0("Data/",x,"/COMUNA_C17.shp"))})
